...
[课程推荐](https://www.bilibili.com/video/BV1KW411u7vy)
...
[root@iZbp11rhvd25b9dwktym67Z ~]# docker ps
CONTAINER ID   IMAGE       COMMAND                  CREATED          STATUS          PORTS                               NAMES
ae0a5dbe5742   mysql:5.7   "docker-entrypoint.s…"   24 minutes ago   Up 24 minutes   0.0.0.0:3306->3306/tcp, 33060/tcp   mysql01
[root@iZbp11rhvd25b9dwktym67Z ~]# docker exec -it ae0a5dbe5742 /bin/bash
root@ae0a5dbe5742:/# mysqladmin --version
...
```shell
root@ae0a5dbe5742:/etc/mysql# vim my.cnf
...
![](https://gitee.com/sun-qiao321/picture/raw/master/images/20210617161155.png)
...
```shell
[root@iZbp11rhvd25b9dwktym67Z ~]# docker stop ae0a5dbe5742
ae0a5dbe5742
[root@iZbp11rhvd25b9dwktym67Z ~]# docker start ae0a5dbe5742
ae0a5dbe5742
[root@iZbp11rhvd25b9dwktym67Z ~]# docker ps
CONTAINER ID   IMAGE       COMMAND                  CREATED             STATUS         PORTS                               NAMES
ae0a5dbe5742   mysql:5.7   "docker-entrypoint.s…"   About an hour ago   Up 3 seconds   0.0.0.0:3306->3306/tcp, 33060/tcp   mysql01

```

重新进入容器，登录mysql 并查看字符集
...
mysql> insert into user value(1,'zhangsan');
...
docker exec -it ae0a5dbe5742 env LANG=C.UTF-8 bash

```shell
[root@iZbp11rhvd25b9dwktym67Z ~]# docker exec -it ae0a5dbe5742 env LANG=C.UTF-8 bash
root@ae0a5dbe5742:/# mysql -uroot -p
Enter password: 
Welcome to the MySQL monitor.  Commands end with ; or \g.
...
![](https://gitee.com/sun-qiao321/picture/raw/master/images/20210617183206.png)
...
![](https://gitee.com/sun-qiao321/picture/raw/master/images/20210617183853.png)
...
![](https://gitee.com/sun-qiao321/picture/raw/master/images/20210617190910.png)
...
**索引的目的**在于提高查询效率，可以类比字典的目录。如果要查`mysql`这个这个单词，我们肯定要先定位到`m`字母，然后从上往下找`y`字母，再找剩下的`sql`。如果没有索引，那么可能需要`a---z`，这样全字典扫描。
...
![](https://gitee.com/sun-qiao321/picture/raw/master/images/20210617201655.png)
...
- `type`：包含 All、index、range、ref、eq_ref、comst、system、null 等
...
![](https://gitee.com/sun-qiao321/picture/raw/master/images/20210617224322.png)

4、查看现在SQL语句的执行计划

![](https://gitee.com/sun-qiao321/picture/raw/master/images/20210617224407.png)
...
![](https://gitee.com/sun-qiao321/picture/raw/master/images/20210617224606.png)
...
![](https://gitee.com/sun-qiao321/picture/raw/master/images/20210617224706.png)

7、当前索引是`idx_article_cv`，来看一下SQL执行计划

![](https://gitee.com/sun-qiao321/picture/raw/master/images/20210617224735.png)
...
![](https://gitee.com/sun-qiao321/picture/raw/master/images/20210617225835.png)

`book`和`class`两张表都是没有使用索引，全表扫描，那么如果进行优化，索引是创建在`book`表还是创建在`class`表呢？下面进行尝试！
...
![](https://gitee.com/sun-qiao321/picture/raw/master/images/20210617230243.png)
...
![](https://gitee.com/sun-qiao321/picture/raw/master/images/20210617230520.png)
...
![](https://gitee.com/sun-qiao321/picture/raw/master/images/20210617231252.png)
...
![](https://gitee.com/sun-qiao321/picture/raw/master/images/20210617231340.png)
...
![](https://gitee.com/sun-qiao321/picture/raw/master/images/20210617233542.png)

全表扫描

![](https://gitee.com/sun-qiao321/picture/raw/master/images/20210617233405.png)
...
+----+-------+-----+---------+---------------------+
|  1 | Ringo |  18 | manager | 2021-06-17 15:23:56 |
+----+-------+-----+---------+---------------------+
1 row in set (0.00 sec)
# 2、使用MySQL内置的函数
...
![](https://gitee.com/sun-qiao321/picture/raw/master/images/20210618084949.png)

![](https://gitee.com/sun-qiao321/picture/raw/master/images/20210618085052.png)
...
![](https://gitee.com/sun-qiao321/picture/raw/master/images/20210618085349.png)

![](https://gitee.com/sun-qiao321/picture/raw/master/images/20210618085521.png)
...
![](https://gitee.com/sun-qiao321/picture/raw/master/images/20210618085758.png)
...
![](https://gitee.com/sun-qiao321/picture/raw/master/images/20210618090148.png)
...
![](https://gitee.com/sun-qiao321/picture/raw/master/images/20210618090547.png)

**口诀：`like`百分加右边。**

如果一定要使用`%like`，而且还要保证索引不失效，那么使用==覆盖索引==来编写SQL。
...
![](https://gitee.com/sun-qiao321/picture/raw/master/images/20210618090820.png)
...
![](https://gitee.com/sun-qiao321/picture/raw/master/images/20210618091951.png)
...
+---------------------+--------------------------------------+
| slow_query_log      | OFF                                  |
| slow_query_log_file | /var/lib/mysql/ae0a5dbe5742-slow.log |
+---------------------+--------------------------------------+
...
root@ae0a5dbe5742:/usr/bin# mysqldumpslow --help
...
# 问题2：SESSION1为mylock表加了写锁，SESSION2写mylock表会阻塞，等待SESSION1释放！
mysql> UPDATE `mylock` SET `name` = 'abc' WHERE `id` = 1;
^C^C -- query aborted
ERROR 1317 (70100): Query execution was interrupted
...
**此外，`MyISAM`的读写锁调度是写优先，这也是`MyISAM`不适合作为 ==以写为主== 表的引擎。因为写锁后，其他线程不能进行任何操作，大量的写操作会使查询很难得到锁，从而造成永远阻塞。**
...
![](https://gitee.com/sun-qiao321/picture/raw/master/images/20210618151934.png)
...
![](https://gitee.com/sun-qiao321/picture/raw/master/images/20210618153152.png)
...
*************************** 1. row ***************************
        ...
        Relay_Master_Log_File: mysql-bin.000001
             Slave_IO_Running: Yes       #### 同时为Yes 则成功
